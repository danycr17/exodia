...
function obtenerTipoCampo($tipo_pregunta, $conf) {
    $conf = htmlspecialchars($conf, ENT_QUOTES, 'UTF-8'); 
    switch ($tipo_pregunta) {
        case 'text':
            return generarInputTexto($conf);
        case 'textarea':
            return generarTextarea($conf);
        case 'radio':
            return generarRadio($conf);
        case 'checkbox':
            return generarCheckbox($conf);
        case 'select':
            return generarSelect($conf);
        default:
            return generarInputTexto($conf); 
    }
}
function generarInputTexto($conf) {
    return "<input type='text' name='respuesta_$conf' required>";
}
function generarTextarea($conf) {
    return "<textarea name='respuesta_$conf' required></textarea>";
}
function generarRadio($conf) {
   
    $opcion = trim($conf);
    return "<input type='radio' name='respuesta_$conf' value='$opcion' required> $opcion<br>";
}
function generarCheckbox($conf) {
    $opcion = trim($conf); 
    return "<input type='checkbox' name='respuesta_{$conf}[]' value='$opcion'> $opcion<br>";
}
function generarSelect($conf) {
    $opcion = trim($conf); 
